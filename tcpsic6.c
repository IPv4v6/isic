...
main(int argc, char **argv)
{
	...
	while((c = getopt(argc, argv, "hd:s:r:m:k:Dp:F:I:u:T:t:vx:")) != EOF){
	  switch (c) {
	   case 'h':
		usage(argv[0]);
		exit(0);
	   ...
	   case 'd':
		...
		if ( strcmp(optarg, "rand") == 0 ) {
			printf("Using random destination IP's\n");
	...
	printf("IPv6 Dst Opts\t= %.0f%%\n", DstOpts);
	printf("Urg Pcnt\t= %.0f%%\t", UrgPct);
	...
	for(acx = 0; acx < num_to_send; acx++) {
		...
		flow	= RAND32;
		nxt	= IPPROTO_TCP;
		tcp = (struct tcphdr *) (buf + IP6_H); /* for no extension header case */
		...
		if ( rand() <= (RAND_MAX * FragPct) ) {
		  ...
		  tcp = (struct tcphdr *) (buf + IP6_H + IP6_FRAGH); /* adjust the pointer */
		...
		if ( rand() <= (RAND_MAX * DstOpts) ) {
			...
			dstopt_len = (int)(9.0*rand()/(RAND_MAX+1.0)); /* maximun 10 x 8 = 80 bytes */
			dstopt_nxt = IPPROTO_TCP;
			dstopt_flag = 1;
			dstopt_tlen = (dstopt_len << 3) + 8;
			packet_len += dstopt_tlen;			
			if (frag_flag) {
				f6_nxt = IPPROTO_DSTOPTS;
				tcp = (struct tcphdr *) (buf + IP6_H + IP6_FRAGH + dstopt_tlen);
				ip6_dopthdr = (struct ip6_dest *)(buf + IP6_H + IP6_FRAGH);
			}
			else {
				nxt = IPPROTO_DSTOPTS;
				tcp = (struct tcphdr *) (buf + IP6_H + dstopt_tlen);
		...
		if ( rand() <= (RAND_MAX * TCPCksm) )
			tcp->th_sum = RAND16;			
		else	libnet_do_checksum(l, (u_int8_t *)buf, IPPROTO_TCP, packet_len - IP6_H);
...
void usage(char *name)
{
   fprintf(stderr,
	...
	"       ex: -s 2001:1:2:3:4::1,23  -d 2001:1:2:3:4::2 -I 100\n"
	"           will give a 100%% chance of IPv6 Destination ^^^ Options \n"
	"       ex: -s 2001:a:b:c:d::1,23  -d 2001:a:b:c:d::2 -p 100 -r 103334\n"
